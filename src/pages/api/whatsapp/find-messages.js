...
export async function GET({ request, cookies }) {
  ...
  try {
    ...
    if (!number) {
      return Response.json({ error: 'Falta parámetro number' }, { status: 400 });
    }
    ...
    if ((!url || !key) && api.isConfigured) {
      try {
        ...
        console.error('[whatsapp-find-messages-proxy] Error al obtener settings de Appwrite:', dbErr.message);
      }
    }

    // 3. Fallback de desarrollo local: si no hay credenciales en servidor, permitir cabeceras custom
    if (!url || !key) {
    ...
    const response = await fetch(`${url}/chat/findMessages/${inst}?number=${cleanNumber}`, {
      method: 'GET',
      headers: {
        'apikey': key,
    ...
    console.error('[whatsapp-find-messages-proxy] Error en proxy:', err.message);
